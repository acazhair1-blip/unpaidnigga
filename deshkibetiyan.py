...
CACHE_TTL = 900  # 15 Minutes Memory Cache (Speed Boost)
...
GLOBAL_CATS = {}
CATS_LAST_FETCH = 0
VID_CACHE = {}  # url -> {'time': timestamp, 'data': []}
USER_VIDS = {}
FILE_CACHE = {}
...
# 🔒 FORCE JOIN CHECKER
# ==========================================
async def is_user_joined_single(client, channel_id, user_id):
    try:
        ...
        return True # Fallback to avoid blocking on API glitches

async def is_user_joined_all(client, user_id):
    res1 = await is_user_joined_single(client, CHANNELS[0]["id"], user_id)
    res2 = await is_user_joined_single(client, CHANNELS[1]["id"], user_id)
...
# ==========================================
# ⚡ HIGH-SPEED CACHED SCRAPERS
# ==========================================
def fetch_cats_sync():
    global CATS_LAST_FETCH
    now = time.time()
    
    # Return Cache if fresh (within 15 mins)
    if GLOBAL_CATS and (now - CATS_LAST_FETCH) < CACHE_TTL:
        return True
        ...
        for a in soup.select('.wp-block-heading a, .entry-content a, .menu-item a, .cat-item a'):
            ...
            if url and SITE_URL in url and len(name) > 2:
                ...
                if not any(x in name.lower() for x in ignore):
                    ...
                    if disp_name not in temp_cats:
                        temp_cats[disp_name] = url
        
        if temp_cats:
            GLOBAL_CATS.clear()
            GLOBAL_CATS.update(temp_cats)
            CATS_LAST_FETCH = now
        return True
    except Exception:
        return False
...
async def fetch_cats():
    return await asyncio.to_thread(fetch_cats_sync)

def fetch_vids_sync(page_url):
    now = time.time()
    
    # Check Video Cache
    if page_url in VID_CACHE:
        cached = VID_CACHE[page_url]
    ...
    try:
        ...
        result = vids[:20]
        VID_CACHE[page_url] = {'time': now, 'data': result}
        return result
    except Exception:
        return []

async def fetch_vids(page_url):
    return await asyncio.to_thread(fetch_vids_sync, page_url)

def extract_media_sync(post_url):
    media = {'videos': [], 'images': []}
    try:
...
async def extract_media(post_url):
    return await asyncio.to_thread(extract_media_sync, post_url)

def download_file_sync(url, file_path):
...
async def download_file(url, file_path):
    return await asyncio.to_thread(download_file_sync, url, file_path)
...
async def search_command(client, message):
    ...
    vids = await fetch_vids(f"{SITE_URL}/?s={requests.utils.quote(query)}")
    if not vids:
...
async def menu_handler(client, message):
    ...
    if text in GLOBAL_CATS:
        ...
        vids = await fetch_vids(GLOBAL_CATS[text])
        if not vids:
...
async def callback_handler(client, query):
    ...
    if data.startswith("play_"):
        ...
        media = await extract_media(vid['url'])
